...
test('cockpit: кандидат Крайнестана доступен для проверки с явным выбором сида', () => {
  const appCode = fs.readFileSync('src/app.js', 'utf8');
  assert.match(appCode, /getScenariosList\(\{\s*all:\s*true\s*\}\)/);
  assert.match(appCode, /data-testid="new-game-seed"/);
});
